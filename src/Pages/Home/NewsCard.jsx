...
const NewsCard = ({ singleNews }) => {
    const { title, image_url, details, author, published_date, _id } = singleNews;
    return (
        <div className="relative flex flex-col bg-white bg-clip-border text-gray-700 shadow-md border">
            ...
            <div className="p-6">
                <p className=" text-base font-medium leading-relaxed">
                    {details.length > 200 ?
                        <p>{details.slice(0, 200)} <Link to={`/news/${_id}`} className="block text-red-500">Read More...</Link> </p>
                        :
                        <p>{details} </p>
                    }
                </p>
